...
Resolve `../../scripts/run-codex-review.mjs` from this skill directory to an absolute path. Each reviewer uses a strict JSON output schema for completion status, canonical finding lines, and a blocker reason. The collector validates the object and generates the canonical completion marker itself. Invalid or contradictory responses remain failures; the collector never guesses that a review completed.
